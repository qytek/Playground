...
**Goal:** Replace the stale Phaser/TypeScript rewrite plan in CLAUDE.md with an accurate, concise document describing the current vanilla JS codebase plus incremental future directions.

**Architecture:** Single-file rewrite. Read the existing CLAUDE.md for context, replace entirely with new content structured in 8 sections per the design spec.
...
Quick-check the file against the source tree:

```bash
ls js/*.js
```

Confirm all 11 JS files listed in the table exist.

```bash
grep -c "// ============" js/*.js
```

Confirm all files use the header comment convention.
...
docs: rewrite CLAUDE.md to reflect current vanilla JS codebase

Replace the stale Phaser/TypeScript rewrite plan with accurate
documentation of the existing project: stack, architecture, file
map, load order, conventions, and incremental future directions.
